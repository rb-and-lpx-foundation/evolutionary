...
def test_nsgaii(context):
    pop = [ChromosomeTestImplementation(context) for i in range(10)]
    assert 3 == len(pop[0].genes)
    nsgaii = NSGAII.NSGAII(context, pop)
...
def test_fast_nondominated_sort(experiment_data):
    def test_rank_list(expected, actual):
        assert 3 == actual[1]
        assert 5 == actual[8]
        assert 11 == actual[58]
        assert expected == actual

    def test_domination_fronts(expected, actual):
        assert expected == actual
    ...
    actual = NSGAII.fast_nondominated_sort(experiment_data)
    test_rank_list(expected[0], actual[0])
    test_domination_fronts(expected[1], actual[1])


def test_crowding_distance(experiment_data):
    expected = {
    ...
    actual = NSGAII.crowding_distance(experiment_data)
    assert expected == actual
